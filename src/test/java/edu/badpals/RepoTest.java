...
public class RepoTest {
    ...
    @Test
    public void test_load_wizard () throws Exception{

        // Hermione no puede comprar items
        Assertions.assertThat(repo).isNotNull();
        try {
            Optional<Wizard> orden = repo.loadWizard("Severus Snape");
        } catch (IllegalArgumentException e) {
            // Verificar el mensaje de la excepción si es necesario
            Assertions.assertThat(e.getMessage()).isEqualTo("Severus Snape no existe");
        }
    ...
    @Test
    public void test_load_item() throws Exception {
        Assertions.assertThat(repo).isNotNull();
        try {
            Optional<MagicalItem> orden = repo.loadItem("Plumbus");
        } catch (IllegalArgumentException e) {
            // Verificar el mensaje de la excepción si es necesario
            Assertions.assertThat(e.getMessage()).isEqualTo("El objeto no fue encontado");
        }
    ...
    @Test
    public void test_load_item_equal() throws Exception{

        Assertions.assertThat(repo).isNotNull();

        try {
            MagicalItem plumbus = new MagicalItem("plumbus", 0, "MagicalItem");
            Optional<MagicalItem> orden = repo.loadItem(plumbus);
        } catch (IllegalArgumentException e) {
            // Verificar el mensaje de la excepción si es necesario
            Assertions.assertThat(e.getMessage()).isEqualTo("El objeto no fue encontado");
    ...
    @Test
    public void test_load_items() throws Exception{
        Assertions.assertThat(repo).isNotNull();

        try {
            List<MagicalItem> orden = repo.loadItems("plumbus");
            Assertions.assertThat(orden.isEmpty());
        } catch (IllegalArgumentException e) {
            // Verificar el mensaje de la excepción si es necesario
            Assertions.assertThat(e.getMessage()).isEqualTo("No se ha encontrado ningun objeto con ese nombre");
        }
